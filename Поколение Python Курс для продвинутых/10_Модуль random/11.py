...
students, friends, couples = [input() for _ in range(int(input()))], set(), {}

for i in range(len(students)):
    temp = {j for j in students} - friends - {students[i]}
    if temp:
        friend = sample(list(temp), 1)
        friends.update(friend)
        couples.update({students[i]: friend})
    else:
        couples.update({students[i]: couples[students[0]]})
        couples[students[0]] = [students[i]]

[print(k, '-', *v) for k, v in couples.items()]
