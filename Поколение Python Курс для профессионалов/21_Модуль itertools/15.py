...
# ,
# 22
# 1,3,5,11,15,22 в отрезок преобразовать нельзя. Одиночное число преобразуется в отрезок, в котором и правой, и левой
# границей является оно само. Например, число
# 1
...
def ranges(numbers):
    group_it = groupby(enumerate(numbers), key=lambda x: x[1] - x[0])
    res = []
    for _, v in group_it:
        temp = tuple(v)
        res.append((temp[0][1], temp[-1][-1]))
    return res


numbers = [1, 2, 3, 4, 7, 8, 10, 3, 4, 5]
