...
Gloria Higgins, James Wilkins, Jane Jackson, John Watts, Kay Scott, Kimberly Reynolds, Linda Hudson, Michelle Wright,
Rosemary Garcia, Steven Diaz
...
staff_dep = defaultdict(set)
for i in staff_broken:
    staff_dep[i[0]].add(i[1])

[print(f"{k}: {', '.join(sorted(staff_dep[k]))}") for k in sorted(staff_dep)]
