"""
Вам доступен список names, содержащий имена на русском языке. Дополните приведенный ниже код, чтобы он вывел все
имена, которые начинаются на буквы А и М (независимо от регистра) и имеют длину больше
4
...
print(*sorted(map(lambda c: c.capitalize(), filter(lambda x: len(x) > 4 and (x.capitalize().startswith('А') or x.capitalize().startswith('М')), names))))
