...
with open('C:/Users/kandz/Downloads/food_services.json', encoding='utf-8') as f1:
    data = json.load(f1)
    food_count = {}
    franch_count = {}
    for d in data:
        food_count[d['District']] = food_count.get(d['District'], 0) + 1
        if d['OperatingCompany']:
            franch_count[d['OperatingCompany']] = franch_count.get(d['OperatingCompany'], 0) + 1
    print('{0[0]}: {0[1]}'.format(max(food_count.items(), key=lambda item: item[1])))
    print('{0[0]}: {0[1]}'.format(max(franch_count.items(), key=lambda item: item[1])))
