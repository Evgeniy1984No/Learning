...
with open('C:/Users/kandz/Downloads/diary.txt', encoding='utf-8') as file:
    file_list = []
    temp_s = ''
    for elem in file.readlines():
        if elem != '\n':
            ...
            file_list.append(temp_s.strip().split('\n'))
            temp_s = ''
    file_list.append(temp_s.split('\n'))
    file_sorted = sorted(file_list, key=lambda x: datetime.strptime(x[0], '%d.%m.%Y; %H:%M'))
