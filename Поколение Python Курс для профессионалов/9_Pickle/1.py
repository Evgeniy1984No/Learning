...
Примечание 2. В тестирующую систему сдайте программу, содержащую только необходимую функцию filter_dump(), но не код,
вызывающий ее.
"""
import pickle
...
def filter_dump(filename: str, objects: list, typename: type):
    with open(filename, 'wb') as file_p:
        filter_objects = list(filter(lambda i: type(i) is typename, objects))
        pickle.dump(filter_objects, file_p)
