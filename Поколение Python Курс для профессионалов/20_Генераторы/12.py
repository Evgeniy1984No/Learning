...
# Если человек жив, год смерти считается равным
# 0
...
filter_death = (person for person in persons if person.death == 0)
filter_sex = (person for person in filter_death if person.sex == 'male')
filter_nation = (person for person in filter_sex if person.nationality == 'Swedish')

print(max(filter_nation, key=lambda p: p.birth)[0])
